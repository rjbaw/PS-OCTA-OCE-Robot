    ...
    if [[ "$sim" == "true" ]]; then
        ...
        tmux new-session -d -s "$TMUX_SESSION" \
            "bash -ic 'source install/setup.bash; \
             ros2 launch octa_ros launch.py ur_type:=ur3e robot_ip:=$HOST headless_mode:=true'"
    else
        HOST="192.168.0.10"
        tmux new-session -d -s "$TMUX_SESSION" \
            "bash -ic 'source install/setup.bash; \
             ros2 launch octa_ros launch.py ur_type:=ur3e robot_ip:=$HOST headless_mode:=true reverse_ip:=192.168.0.2'"
    fi

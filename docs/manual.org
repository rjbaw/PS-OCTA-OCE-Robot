...
Project directory is located in the home location =/home/bail/=
[[./directory.png]]
...
Images should be inside =result/= inside the project directory. They do not automatically clear so you have to routinely clean it up.
...
Robot must be in remote control mode for the driver to work.

[[./remote_control.png]]

Networking must also be set to manual mode with the following settings:
...
# Logs
make logs         # summarize the newest session and create a support bundle
...
Make targets:
  ...
  logs   - Summarize the latest session and create a support bundle
...
For manager and node diagnostics, run ~make logs~ from the project root on the host.
...
1. Restart the robot driver by toggling =Robot ON/OFF= between the ON and OFF state.
2. Restart LabVIEW and run =make restart=.
3. Run ~make logs INCIDENT=error~ and send the generated support bundle to the maintainer.

** Log Reporting
*** Quick view
#+ATTR_LATEX: :options frame=lines, linenos
#+begin_src bash
make logs                    # newest coordinator session
make logs INCIDENT=auto      # newest incident, even if an operational session is newer
make logs INCIDENT=fullscan  # newest Fullscan session
make logs INCIDENT=error     # newest genuine crash/error
make logs SESSION=1          # previous matching session (SESSION=0 is newest)
make logs VIEW=raw           # contiguous original lines from that coordinator log
#+end_src

By default, ~make logs~ selects the newest coordinator session, and ~SESSION=0~ selects the newest session matching any explicit filter. Use ~INCIDENT=auto~ to search incident-bearing sessions even when a newer clean session exists. The output identifies the selected session and shows the oldest and newest matching sessions in chronological order. The command prints a short summary and creates a bounded =.tar.gz= support bundle under =./logs/support-bundles/=. ~LOG_MAX_BUNDLE_BYTES~ limits the final compressed archive size; if necessary, raw logs are omitted and recorded in =manifest.txt=. It does not merge event lines from different processes. ~VIEW=raw~ prints one contiguous, unfiltered clip from the selected coordinator INFO log, with its source path and physical line boundaries. Increase the clip with ~LOG_RAW_LINES=4000~ if needed. Exact known-benign cancellation diagnostics are ignored for incident selection but remain present in raw logs. A generic action =CANCELED= result remains visible because the log alone cannot reliably distinguish a user request from an action-server disconnect.
...
- The =.tar.gz= archive printed by =make logs=.
- Review =runtime/context.txt= and =runtime/docker.log= before sharing outside the support team; they may contain configured IP addresses, dirty-file names, or application output.
...
- =LAUNCH_RVIZ=: launch RViz with the managed or debug stack (default =false=; =make test= sets =true=)
...
- Key parameters:
  ...
  - =scan_trigger_timeout_sec= (double, default 10.0): max time a scan trigger can stay active before it is cleared.
  - =scan3d_window_ms= (int, default 50): post-acknowledgement settling delay before Scan3D activation succeeds.
- Fullscan waits 350 ms after LabVIEW reports a changed Robot/OCT/OCT-A/OCE mode before dispatching the next recipe action. An already-stable mode proceeds immediately.

These coordinator parameters are read during node initialization; restart the node after changing them.
...
- Parameters:
  - =plan_only= (bool, default =false=): when =true=, no MoveIt execution (used for CI and simulation; restart required to change).
...
- Tips:
  - Focus timing, image, calibration, and model parameters are read at startup; restart the node after changing them.
...
- Action: =move_action= with goal fields (=offset_x=, =offset_y=, =target_angle=, =radius=, =apply_offset=).
- Parameters:
  - =plan_only= (bool, default =false=): when =true=, skip planning and execution (restart required to change).
...
** Reset (=reset_node=)
- Role: returns the robot to a known reset posture with URScript and confirms completion from the monitored joint state within 30 seconds.
- Parameters:
  - =plan_only= (bool, default =false=): when =true=, skip the robot command and state confirmation (restart required to change).
  - =urscript_robot_vel= (double, default =0.5=): velocity used for the reset =movej=.
  - =urscript_robot_acc= (double, default =0.5=): acceleration used for the reset =movej=.
...
- Parameters:
  ...
  - =dry_run= (bool, default =false=): skip controller manager calls (used in CI and dev without hardware; restart required to change).
